...
public class adm_agregarAsignaturaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        RequestDispatcher respuesta = request.getRequestDispatcher("/adm_agregarAsignatura.jsp");
        respuesta.forward(request,response);
    }

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException  {
        RequestDispatcher respuesta = req.getRequestDispatcher("/adm_errorAgregarAsignatura.jsp");
        if(req.getParameter("nombre_asignatura").length() != 0 || req.getParameter("modulo").length() != 0
            ...
            try {
                if(agregarAsignatura(asignatura)){
                    ...
                    respuesta = req.getRequestDispatcher("/adm_exitoAgregarAsignatura.jsp");
                }
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
            }
