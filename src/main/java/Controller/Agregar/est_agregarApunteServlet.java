...
public class est_agregarApunteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        RequestDispatcher respuesta = request.getRequestDispatcher("/est_agregarApunte.jsp");
        respuesta.forward(request,response);
    }
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException  {
        RequestDispatcher respuesta = req.getRequestDispatcher("/est_errorAgregarApunte.jsp");
        if(req.getParameter("nombre_apunte").length() != 0 || req.getParameter("cod_asignatura").length() != 0
            ...
            try {
                if(agregarApunte(apunte)){
                    ...
                    respuesta = req.getRequestDispatcher("/est_exitoAgregarApunte.jsp");
                }
            } catch (ClassNotFoundException e) {
                e.printStackTrace();
            }
